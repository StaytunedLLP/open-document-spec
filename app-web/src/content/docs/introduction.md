...
**Automation default:** child lists in `index.md` are generated by `odc ods index` / `odc ods start` / `odc ods watch`. Do not hand-edit those bullets—set document `description` instead.
...
- **Navigable trees** — Generated `index.md` files stay current under `odc ods start` / `watch` / `index`.
