...
4. **Semantic Codebase & Child Folder Rearrangement under `src/`**:
   - Re-organize `src/crates/` into semantically divided crates and domain subfolders:
     - `src/crates/ods-core`: Engine core (`model/`, `parse/`, `profiles/`, `lint/`, `graph/`, `lifecycle/`, `fs/`, `bench/`, `export/`).
     - `src/crates/ods-cli`: CLI binary & command runners (`cli/`, `commands/`, `service/`, `update/`, `support/`).
     - `src/crates/ods-test-support`: Workspace test harness & test fixtures.
