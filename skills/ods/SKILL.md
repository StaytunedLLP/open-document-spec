...
**Root `index.md` only:** `odc` (legacy: `ods`) (spec/schema version), `ods-cli` (CLI
...
`ods: 0.1` and `ods-cli: ">=0.0.1"` once — `ods:` is the workspace marker and
...
- `odc ods init [path]` (alias `enable`) — make folder/repo ODS-compliant (create root `index.md` + `ods:` / `ods-cli:` markers, index). `--adopt` also drafts frontmatter on plain files.
...
- `odc ods lint [path]` — validate; `--level 1|3`, `--format text|json`, `--canonical-refs` (warn on extensionless Document refs). Writes/clears `ods-error.md`.
...
- `odc workspaces list|add|remove|path` — manage globally tracked workspace paths in `~/.ods/odsconfig.toml` (legacy `~/.ods/workspaces.toml` auto-migrates on read).
