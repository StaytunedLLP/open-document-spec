...
| Command | Action |
|---|---|
| **`ods lint`** | Validates structural integrity, graph links, and profile H2/H3 section trees. |
| **`ods lint --okf`** | Validates OKF document bundles using the `--okf` flag. |
| **`ods new <profile> <path>`** | Scaffolds a new document using the profile's template. |
| **`ods profile list`** | Lists all standard 12 profiles + registered custom profiles. |
...
| **`ods init`** | Initialized root `index.md` (`ods: 0.1`). |
