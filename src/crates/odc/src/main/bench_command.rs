fn run_bench_command(args: &[String]) -> Result<ExitCode, CliError> {
    ...
    match sub {
        "strip" => {
            ...
            let options = odc_core::BenchStripOptions {
                write,
                path_filter,
                strip_indexes,
                strip_profiles,
            ...
            let report = odc_core::bench_strip_workspace(&root, options)
        ...
        "restore" => {
            ...
            let report = odc_core::bench_restore_workspace(&root, snapshot_id.as_deref())
                .map_err(|err| failure(err.to_string()))?;
        ...
        "stats" | "roi" => {
            ...
            let stats = odc_core::bench_calculate_stats(&root)
                .map_err(|err| failure(err.to_string()))?;
