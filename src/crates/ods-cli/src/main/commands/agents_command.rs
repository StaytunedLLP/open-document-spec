...
fn run_agents_sync_command(args: &[String]) -> Result<ExitCode, CliError> {
    ...
    fs::write(
        ...
        "# Open Document Spec agent notes\n\nSee [AGENTS.md](../AGENTS.md) generated by `ods agents sync`.\n\n\
         Use `ods ods context <id>` for ODS reading lists and `ods okf context <id>` for OKF.\n",
    )
    ...
    fs::write(
        ...
        "---\ndescription: Open Document Spec multi-spec agent hints\nglobs:\nalwaysApply: false\n---\n\n\
         Prefer `ods ods` for codebase docs and `ods okf` for OKF knowledge bundles. \
         See root AGENTS.md.\n",
    )
    .map_err(|e| failure(e.to_string()))?;
    ...
    if ods_core::ods_enabled(root) {
        md.push_str(
            "- **ODS** workspace (`ods:` root marker)\n\
             - Lint: `ods ods lint`\n\
             - Context: `ods ods context <id>`\n\
             - Audit: `ods ods audit --write-report`\n\n",
        );
    }
    if ods_core::okf_enabled(root) {
        md.push_str(
            "- **OKF v0.2** bundle (`okf_version` root marker)\n\
             - Lint: `ods okf lint`\n\
             - Context: `ods okf context <id>`\n\
             - Audit: `ods okf audit --write-report`\n\n",
        );
    }
    if !ods_core::ods_enabled(root) && !ods_core::okf_enabled(root) {
        md.push_str(
            "- No ODS/OKF root markers detected. Run `ods ods init` or `ods okf init`.\n\n",
        );
    }
    md.push_str(
        "## Rules for coding agents\n\n\
         1. Prefer `ods …` (auto-detect) or explicit `ods ods …` / `ods okf …` in CI.\n\
         2. ODS Markdown keys stay `ods:` / `ods:` — do not invent `ods-cli:`.\n\
         ...
         5. After structural doc edits: `ods ods lint` or `ods okf lint` as appropriate.\n",
    );
    md
}
