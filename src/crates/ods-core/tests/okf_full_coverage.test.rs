...
fn scaffold_and_remove_profile_templates_and_errors() {
    ...
    for (prof, file) in [
        ...
        let rep = ods_core::scaffold_new_document(
            root,
            std::path::Path::new(file),
            ods_core::NewDocumentOptions {
    ...
    // AlreadyExists error
    let err_exist = ods_core::scaffold_new_document(
        root,
        std::path::Path::new("decisions/adr.md"),
        ods_core::NewDocumentOptions::default(),
    );
    assert!(err_exist.is_err());

    // NotFound error in delete
    let err_nf = ods_core::atomic_delete_document(
        root,
        std::path::Path::new("nonexistent.md"),
        ods_core::RemoveDocumentOptions::default(),
    );
