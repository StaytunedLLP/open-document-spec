...
/// Path to the global ODS machine configuration file (`~/.ods/odsconfig.toml`).
pub fn registry_path() -> PathBuf {
    let home = env::var("HOME").unwrap_or_else(|_| ".".into());
    let primary = PathBuf::from(&home).join(".ods/odsconfig.toml");
    if primary.exists() {
        return primary;
    }
    let fallback = PathBuf::from(&home).join(".ods/config.toml");
    if fallback.exists() {
        return fallback;
    }
    primary
