...
fn install_release(tag: &str, target: &str, prefix: &Path) -> Result<(), String> {
    ...
    let archive_id = find_asset_id(&release_json, &filename).ok_or_else(|| {
        format!("asset {filename} not found on release {tag} (check GH_TOKEN for private repo)")
    })?;
    ...
    let ods_src = find_ods_binary(&extract_dir, is_windows_target(target))?;
    ...
    replace_binary(&ods_src, &prefix.join(ods_name))?;
